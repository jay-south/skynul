...
export class SlackChannel extends Channel {
  ...
  async start(): Promise<void> {
    ...
    const botToken = this.opts.credentials.botToken
    const appToken = this.opts.credentials.appToken
    if (!botToken || !appToken) return

    this.slackApp = new SlackApp({ token: botToken, appToken, socketMode: true })
  ...
  async setCredentials(creds: Record<string, string>): Promise<void> {
    if (creds.botToken) this.opts.credentials.botToken = creds.botToken.trim()
    if (creds.appToken) this.opts.credentials.appToken = creds.appToken.trim()
  }
  ...
  protected async sendMessage(text: string): Promise<void> {
    ...
    try {
      await this.slackApp.client.chat.postMessage({ channel: this.state.pairedChannelId, text })
    } catch {
      /* skip */
    }
  }
  ...
  private async handleIncoming(
    ...
    if (content.startsWith('/pair ')) {
      ...
      if (!this.state.pairingCode) {
        await say('No hay código activo.')
        return
      }
    ...
    this.emitIncoming(userId, content)
    await say('✅ Tarea enviada!')
  }
