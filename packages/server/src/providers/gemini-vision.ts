...
export async function geminiVisionRespond(opts: {
  ...
  if (!opts.apiKey) throw new Error('Gemini API key is not set.')
  ...
  const url = `${GEMINI_BASE_URL}/models/${encodeURIComponent(GEMINI_VISION_MODEL)}:generateContent?key=${encodeURIComponent(opts.apiKey)}`
