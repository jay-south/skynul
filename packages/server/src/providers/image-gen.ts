...
function getFirstKey(...keys: string[]): string | undefined {
  return process.env[keys.find((k) => process.env[k]) ?? '']
}
...
export async function generateImage(
  ...
  const openaiKey = getFirstKey('OPENAI_API_KEY', 'openai.apiKey')
  if (openaiKey) return generateWithDalle(prompt, size, openaiKey)

  const geminiKey = getFirstKey('GEMINI_API_KEY', 'gemini.apiKey')
