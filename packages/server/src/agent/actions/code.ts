...
export async function handleFileRead(action: TaskAction): Promise<string> {
  ...
  const raw = $(action)
  const filePath = raw.cwd
    ? path.resolve(raw.cwd as string, raw.path as string)
    : path.resolve(raw.path as string)
  try {
...
export async function handleFileEdit(action: TaskAction): Promise<string> {
  ...
  try {
    ...
    if (count > 1)
      return `[Error: old_string found ${count} times in ${filePath} — must be unique. Add more context.]`
...
export async function handleFileList(action: TaskAction): Promise<string> {
  ...
  return new Promise((resolve) => {
    ...
    exec(
      fdCmd,
      { timeout: 10_000, maxBuffer: 512 * 1024, cwd: raw.cwd as string | undefined },
      (err, stdout) => {
        if (!err && stdout.trim()) {
          resolve(headTail(stdout.trim(), 6000))
          return
        }
    ...
    exec(
      ...
      (err, stdout) => {
        if (!err || (err as { code?: unknown }).code === 1) {
          resolve(headTail((stdout || '').trim() || '(no matches found)', 6000))
          return
        }
        const grepGlob = raw.glob
          ? ` --include='${(raw.glob as string).replace(/'/g, "'\\''")}'`
          : ''
        ...
        exec(
          ...
          { timeout: 10_000, maxBuffer: 512 * 1024, cwd: raw.cwd as string | undefined },
          (err2, stdout2) => {
            if (err2 && !(err2 as { code?: unknown }).code) {
