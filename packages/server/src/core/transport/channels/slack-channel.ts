...
import { get, set } from '../../repositories/secrets'
...
export class SlackChannel extends Channel {
  ...
  async start(): Promise<void> {
    ...
    const appToken = get('slack.appToken')
  ...
  async setCredentials(creds: Record<string, string>): Promise<void> {
    if (creds.botToken) set('slack.botToken', creds.botToken.trim())
    if (creds.appToken) set('slack.appToken', creds.appToken.trim())
  }
