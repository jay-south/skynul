...
export class PolymarketClient {
  ...
  private async getLiveSdkClient(): Promise<unknown> {
    const { get } = await import('../../repositories/secrets')
    const pk = (get('POLYMARKET_PRIVATE_KEY')) ?? process.env.POLYMARKET_PRIVATE_KEY
    const funder =
      (get('POLYMARKET_FUNDER_ADDRESS')) ?? process.env.POLYMARKET_FUNDER_ADDRESS
    const sigTypeRaw =
      (get('POLYMARKET_SIGNATURE_TYPE')) ?? process.env.POLYMARKET_SIGNATURE_TYPE ?? '2'
