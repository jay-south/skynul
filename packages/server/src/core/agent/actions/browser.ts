...
export async function handleNavigate(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
  ...
  if (isImgSite) {
    ...
    if (matchedSite) {
      const isExplicit = promptLower.includes('bing') || promptLower.includes('image') || promptLower.includes('create')
      if (!isExplicit) {
...
export async function handleClick(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
  await ctx.engine.click((action as Record<string, unknown>).selector as string, (action as Record<string, unknown>).frameId as string | undefined)
  return undefined
}

export async function handleType(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
  const raw = action as Record<string, unknown>
  await ctx.engine.type(raw.selector as string, raw.text as string, raw.frameId as string | undefined)
  return undefined
}

export async function handlePressKey(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
  await ctx.engine.pressKey((action as Record<string, unknown>).key as string)
  return undefined
}

export async function handleEvaluate(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
...
export async function handleUploadFile(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
  const raw = action as Record<string, unknown>
...
export async function handleScroll(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
  const dir = (action as Record<string, unknown>).direction as string
  await ctx.engine.evaluate(`window.scrollBy(0, ${dir === 'up' ? -400 : 400})`)
  return undefined
}

export async function handleScrollIntoView(action: TaskAction, ctx: BrowserContext): Promise<string | undefined> {
  const raw = action as Record<string, unknown>
  const sel = (raw.selector as string).replace(/'/g, "\\'")
  await ctx.engine.evaluate(`document.querySelector('${sel}')?.scrollIntoView({block:'center',behavior:'instant'})`, raw.frameId as string | undefined)
  return undefined
}
